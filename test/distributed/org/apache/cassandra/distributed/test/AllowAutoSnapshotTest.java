...
public class AllowAutoSnapshotTest extends TestBaseImpl
{
    ...
    public void testTruncateAllowAutoSnapshot() throws Exception
    {
        try (Cluster c = getCluster(true))
        {
            ...
            c.coordinator(1).execute(withKeyspace("INSERT INTO %s.test_table (a, b) VALUES ('a', 1);"), ConsistencyLevel.ONE);
            ...
            c.coordinator(1).execute(withKeyspace("INSERT INTO %s.test_table2 (a, b) VALUES ('a', 1);"), ConsistencyLevel.ONE);
    ...
    {
        return init(build(2).withConfig(c -> c.with(GOSSIP).set("auto_snapshot", autoSnapshotEnabled)).start());
    }
