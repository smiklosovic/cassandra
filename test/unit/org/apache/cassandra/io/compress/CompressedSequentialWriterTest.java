...
public class CompressedSequentialWriterTest extends SequentialWriterTest
{
    ...
    private void testWrite(File f, int bytesToTest, boolean useMemmap) throws IOException
    {
        ...
        assert f.exists();
        try (FileHandle.Builder builder = new FileHandle.Builder(filename).withCompressionMetadata(new CompressionMetadata(filename + ".metadata", f.length(), true, false));
    ...
    private void testUncompressedChunks(int size, double ratio, int extra) throws IOException
    {
        ...
        try (CompressedSequentialWriter writer = new CompressedSequentialWriter(null, f, f.path() + ".metadata",
                                                                                null, SequentialWriterOption.DEFAULT,
                                                                                compressionParameters,
                                                                                sstableMetadataCollector))
        {
    ...
    public void resetAndTruncateTest()
    {
        ...
        try (SequentialWriter writer = new CompressedSequentialWriter(null, tempFile, offsetsFile.path(),
                                                                      null, SequentialWriterOption.DEFAULT,
    ...
    private static class TestableCSW extends TestableSW
    {
        ...
        {
            this(file, offsetsFile, new CompressedSequentialWriter(null, file, offsetsFile.path(),
                                                                   null, SequentialWriterOption.DEFAULT,
