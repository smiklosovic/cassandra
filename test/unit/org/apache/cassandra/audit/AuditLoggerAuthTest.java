...
public class AuditLoggerAuthTest
{
    ...
    public void testCqlALTERRoleAuditing()
    {
        ...
        assertLogEntry(logEntry, AuditLogEntryType.ALTER_ROLE, "ALTER ROLE " + TEST_ROLE + " WITH PASSWORD = '" + new QueryEvents.PasswordObfuscator().getObfuscationToken() + "'", CASS_USER);
        assertEquals(0, getInMemAuditLogger().size());
    }
    ...
    private static String getCreateRoleCql(String role, boolean login, boolean superUser, boolean isPasswordObfuscated)
    {
        return String.format("CREATE ROLE IF NOT EXISTS %s WITH LOGIN = %s AND SUPERUSER = %s AND PASSWORD = '%s'",
                             role, login, superUser, isPasswordObfuscated ? new QueryEvents.PasswordObfuscator().getObfuscationToken() : TEST_PW);
    }
