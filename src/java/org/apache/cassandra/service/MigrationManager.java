...
public class MigrationManager
{
    ...
    public static void scheduleSchemaPullNoDelay(InetAddress endpoint, EndpointState state, MigrationTask.MigrationTaskCallback cb)
    {
        ...
        if (!endpoint.equals(FBUtilities.getBroadcastAddress()) && schemaVersion != null)
            maybeScheduleSchemaPull(schemaVersion, endpoint, state.getApplicationState(ApplicationState.RELEASE_VERSION).value, true, cb);
    }

    public static void scheduleSchemaPull(InetAddress endpoint, EndpointState state, MigrationTask.MigrationTaskCallback cb)
    {
        if (state == null)
            return;
        ...
        UUID schemaVersion = state.getSchemaVersion();
        if (!endpoint.equals(FBUtilities.getBroadcastAddress()) && schemaVersion != null)
            maybeScheduleSchemaPull(schemaVersion, endpoint, state.getApplicationState(ApplicationState.RELEASE_VERSION).value, false, cb);
    }

    public static void scheduleSchemaPull(InetAddress endpoint, EndpointState state)
    {
        scheduleSchemaPull(endpoint, state, new MigrationTask.MigrationTaskCallback(endpoint));
    }
                                                ...
                                                MigrationTask.MigrationTaskCallback cb)
    {
        ...
        if (Schema.instance.isSameVersion(theirVersion) && !cb.isRunningForcibly())
        {
